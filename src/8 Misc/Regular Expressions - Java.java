...
public class LogLan {
  ...
  public static void main (String args[]){
    ...
    while (true) {
      ...
      // where the last line is terminated by a period. The code below
      // reads lines until encountering a line whose final character is
      // a '.'.  Note the use of
      //
      //  s.length() to get length of string
      //  s.charAt() to extract characters from a Java string
      //  s.trim() to remove whitespace from beginning/end of string
      //
      // Other useful String manipulation methods include
      //  s.compareTo(t) < 0 if s < t, lexicographically
      //  s.indexOf("apple") returns index of first occ. of "apple"
      //  s.lastIndexOf("apple") returns index of last occ. of "apple"
      //  s.replace(c,d) replaces occurrences of character c with d
      //  s.startsWith("apple) returns (s.indexOf("apple") == 0)
      //  s.toLowerCase() / s.toUpperCase() returns a new string
      //
      ...
      if (pattern.matcher (removed_period).find()) System.out.println ("Good");
